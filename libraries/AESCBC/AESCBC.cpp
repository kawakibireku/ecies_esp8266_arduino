...
void SHA256X::SHA256_Process(const byte digest[64])
{
 ...
 */
void SHA256X::doUpdate(const byte * msg, int len)
{
 ...
 */
void SHA256X::doFinal(byte *digest)
{
    uint32_t last, padn;
    uint32_t high, low;
    byte msglen[8];
    ...
    SHA256X::doUpdate(sha256_padding, padn);
    SHA256X::doUpdate(msglen, 8);
...
bool SHA256X::matches(const byte *expected)
{
...
SHA256HMACX::SHA256HMACX(const byte *key, unsigned int keyLen)
{
    // sort out the key
...
void SHA256HMACX::doUpdate(const byte *msg, unsigned int len)
{
    _hash.doUpdate(msg, len);
}

void SHA256HMACX::doFinal(byte *digest)
{
...
bool SHA256HMACX::matches(const byte *expected)
{
    byte theDigest[SHA256_SIZE];
    doFinal(theDigest);
    for (byte i = 0; i < SHA256_SIZE; i++)
...
void SHA256HMACX::blockXor(const byte *in, byte *out, byte val, byte len)
{
